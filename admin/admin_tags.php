...
// Lấy danh sách tag
$tags = $conn->query("SELECT * FROM tags ORDER BY tag_id DESC");
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="form-section">
        <form method="post">
            <?php if (isset($_GET['edit_tag'])): ?>
                <input type="hidden" name="tag_id" value="<?= $_GET['edit_tag'] ?>">
                <label>Sửa tag:</label><br>
            <?php else: ?>
                <label>Thêm tag mới:</label><br>
            <?php endif; ?>
            Tên tag: <input type="text" name="name" required value="<?= htmlspecialchars(isset($tag_name) ? $tag_name : '') ?>"><br>
            <?php if (isset($_GET['edit_tag'])): ?>
                <button class="btn btn-warning" name="edit_tag" type="submit">Cập nhật</button>
                <a class="btn" href="admin_tags.php">Hủy</a>
            <?php else: ?>
                <button class="btn" name="add_tag" type="submit">Thêm tag</button>
            <?php endif; ?>
        </form>
    </div>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <a class="btn" href="admin_tags.php?edit_tag=<?= $tag['tag_id'] ?>">Sửa</a>
                <a class="btn btn-danger" href="admin_tags.php?delete_tag=<?= $tag['tag_id'] ?>" onclick="return confirm('Xoá tag này?')">Xoá</a>
            </td>
        </tr>
        <?php endwhile; ?>
    </table>
